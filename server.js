...
// OpenAI-compatible chat call to Groq
...
app.post("/api/lie/generate", async (req, res) => {
  try {
    ...
    const messages = [
      {
        role: "system",
        content:
          "Create a Spot-the-Lie quiz. Produce exactly 4 short statements about the given topic: 3 truths and 1 lie. Return strict JSON with keys: statements (array of 4 strings), lieIndex (1-4), hint (short helpful hint). Do NOT add extra text.",
      },
    ...
    try {
      ...
      parsed = {
        statements: [raw, "Option B", "Option C", "Option D"],
        lieIndex: 1,
        hint: "One of these looks suspicious.",
      };
    }
    const token = makeId();
    sessions.set(token, {
      lieIndex: parsed.lieIndex,
...
/* ========== Game 3: Player asks questions, AI answers ========== */
// Start session: pick a secret character related to topic (avoid repeats per topic)
app.post("/api/character/start", async (req, res) => {
  try {
    ...
    try {
      ...
      // keep defaults on JSON failure
    }

    const rec = recentByTopic.get(topic) || [];
    const lowerRec = rec.map((x) => x.toLowerCase());
    const name = candidates.find((c) => !lowerRec.includes(c.toLowerCase())) || candidates[0];
    ...
    res.json({
      ...
      sessionId: id,
      message:
        "Ask yes/no questions about the secret character. You have 10 rounds. Natural guesses are accepted.",
    });
  } catch (e) {
    res.status(500).json({ ok: false, error: e.message });
  }
...
// Turn: user asks a question or implicitly guesses; AI replies and we track rounds/hints
app.post("/api/character/turn", async (req, res) => {
  try {
    ...
    const messages = [
      {
        role: "system",
        content: `You are running a 20-questions style game. The secret answer is "${s.name}".
