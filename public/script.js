...
======================== */
(function(){
  const form = document.getElementById('riddle-start');
  if(!form) return;
  ...
  const hintBadge = document.getElementById('riddle-hint-badge');
  const textEl = document.getElementById('riddle-text');
  const answerForm = document.getElementById('riddle-answer-form');
  const guess = document.getElementById('riddle-guess');
  const hintBtn = document.getElementById('riddle-hint-btn');
  const skipBtn = document.getElementById('riddle-skip-btn');
  const feedback = document.getElementById('riddle-feedback');
  let token = null, idx = 1, score = 0, busy = false, hintUsed = false;

  const setFeedback = (msg) => { feedback.style.display = 'block'; feedback.textContent = msg; };
  const render = (json) => {
    idx = json.idx || idx; score = json.score ?? score;
    status.textContent = `Riddle ${idx} of ${json.total || 5}`;
    scoreEl.textContent = `Score: ${score}`;
    textEl.textContent = json.riddle || '';
    guess.value = '';
    hintUsed = false;
    hintBadge.textContent = 'Hint: available';
  ...
  async function action(action, value=''){
    ...
    busy = true;
    try {
      const res = await fetch('/api/riddle/action', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({token, action, guess:value}) });
      const json = await res.json();
      if(!json.ok) { setFeedback('Error: ' + (json.error || 'Unknown error')); return; }
      setFeedback(json.feedback || '');
      if(action === 'hint') { hintUsed = true; hintBadge.textContent = 'Hint used'; hintBtn.disabled = true; return; }
      if(json.done) {
        setFeedback(`${json.feedback || ''}\n\n🏁 ${json.message || `Final score: ${json.score}/5`}`);
        token = null;
  ...
  form.addEventListener('submit', async (e)=>{
    ...
    try {
      ...
      const res = await fetch('/api/riddle/start', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({theme}) });
      const json = await res.json();
      if(!json.ok) { setFeedback('Error: ' + (json.error || 'Unknown error')); return; }
      token = json.token; idx = 1; score = 0;
  ...
  answerForm?.addEventListener('submit', e=>{ e.preventDefault(); action('answer', guess.value.trim()); });
  hintBtn?.addEventListener('click', ()=>action('hint'));
  skipBtn?.addEventListener('click', ()=>action('skip'));
})();
